...
Runs the Screenpipe activity controller and serves a web dashboard at localhost:45139.
...
def log_screenpipe(line: str):
    entry = f"[{timestamp()}] {line}"
    screenpipe_log_buffer.append(entry)
    if len(screenpipe_log_buffer) > 200:
        screenpipe_log_buffer.pop(0)
...
# --- Screenpipe log drain ---

def drain_screenpipe_logs(proc):
    """Read stderr from screenpipe into the screenpipe log buffer."""
    try:
...
def start_screenpipe():
    ...
    with lock:
        ...
        threading.Thread(
            target=drain_screenpipe_logs,
            args=(screenpipe_proc,),
            daemon=True,
        ).start()
        is_paused = False
...
def get_screenpipe_logs():
    return {"logs": screenpipe_log_buffer[-50:]}
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .log-entry.sp { color: #444; }
  .log-entry.sp.recent { color: #5a6a7a; }
...
<div class="logs-grid">
  <div>
    <div class="logs-header">
      <span class="section-label">C2 Log</span>
    </div>
    <div class="log-box" id="log-box"></div>
  </div>
  <div>
    <div class="logs-header">
      <span class="section-label">Screenpipe Output</span>
    </div>
    <div class="log-box" id="sp-log-box"></div>
  </div>
</div>
...
async function refresh() {
  const [status, logs, spLogs] = await Promise.all([
  ...
  logBox.innerHTML = logs.logs.slice().reverse().map((e, i) =>
    `<div class="log-entry ${i < 5 ? 'recent' : ''}">${e}</div>`
  ).join('');

  // Screenpipe log
  const spLogBox = document.getElementById('sp-log-box');
  const spEntries = spLogs.logs;
  if (spEntries.length === 0) {
    spLogBox.innerHTML = '<div class="log-entry gray">no output yet</div>';
  } else {
    spLogBox.innerHTML = spEntries.slice().reverse().map((e, i) =>
      `<div class="log-entry sp ${i < 5 ? 'recent' : ''}">${e}</div>`
    ).join('');
  }
}
